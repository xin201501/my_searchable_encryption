...
class TestSortEncryptedResults:
    ...
    async def test_normal_case(self, mock_sort):
        """
        测试正常排序流程
        ...
        valid_request = SortRequest(
            encrypted_results=[("item1", "item2")], index_key="test_key"
        )
        ...
        mock_sort.assert_called_once_with(
            enc_result_list=[("item1", "item2")], index_key="test_key"
        )

    @pytest.mark.asyncio
    async def test_invalid_request_structure(self):
        """
        测试请求体结构异常
        场景：缺少必填字段index_key
        """
        ...
        with pytest.raises(ValidationError):
            invalid_request = SortRequest(
                encrypted_results=[("item1", "item2")], index_key=None
            )
            await sort_encrypted_results(invalid_request)

    @pytest.mark.asyncio
    @patch("sort_enc_result.sort_enc_result", spec=True)
    async def test_sort_function_failure(self, mock_sort):
        """
        测试排序函数内部异常
    ...
    async def test_edge_case_empty_list(self, mock_sort):
        ...
        # 根据业务需求调整断言条件
        request = SortRequest(encrypted_results=[], index_key="test_key")
        response = await sort_encrypted_results(request)
        assert response == {"sorted_results": []}
        mock_sort.assert_called_once_with(enc_result_list=[], index_key="test_key")
