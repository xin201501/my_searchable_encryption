...
class TestEncryptedSearchEngine:
    ...
    def test_empty_documents(self):
        """场景1: 空文档集合验证"""
        self.engine._EncryptedSearchEngine__count_keyword_appearance() # type: ignore
        assert self.engine.words_appearance_time == defaultdict(
            int
        )
    ...
    def test_multiple_scenarios(self, input_data, expected):
        ...
        self.engine._EncryptedSearchEngine__count_keyword_appearance() # type: ignore
        assert (
            dict(self.engine.words_appearance_time) == expected
        )
    ...
    def test_large_dataset(self):
        ...
        self.engine._EncryptedSearchEngine__count_keyword_appearance() # type: ignore
        # 验证累计结果
        assert self.engine.words_appearance_time["word"] == sum(
            range(1000)
        )
